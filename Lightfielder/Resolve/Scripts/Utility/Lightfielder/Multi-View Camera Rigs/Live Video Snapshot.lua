_VERSION = '2026-09-19'
--[[--
Lightfielder Live Video Snapshot 2026-09-19 01.06 AM (UTC -3)
by Andrew Hazelden <andrew@andrewhazelden.com>


# Overview

The "Lightfielder Live Video Snapshot" script allows you to capture imagery from an HDMI/SDI/USB/Thunderbolt video capture device using the MacOS (AV Foundation) and Windows (DirectShow) libraries. The script has a UI Manager GUI and uses FFMPEG from the command line to do the capture task.

The script is compatible with Fusion 9-21.1+ and Resolve 15-21.1+ .

Usage

Step 1. Select the "Utility > Lightfielder > Tools > Live Video Snapshot" menu item to launch the script.

Step 2. On the top row of the Live Video Snapshot window you can select the "Video Input Device", then you can select the captured "Media Type", "Resolution", "FPS" (Frame Rate), and image buffer "Format". Make sure to adjust the "Capture Duration" value to define if you want single frame or multi-frame recording.
...
local captureFolder = 'Temp:/Lightfielder/' .. tostring(paddedImageNumber) .. /'
...
print('[Lightfielder Live Video Snapshot] ' .. _VERSION)
print('----------------------------------------')
...
if not comp then
	print('[Lightfielder][Comp Error] Please run this script from inside the Fusion page.')

	return
end

-- Where should the frame captures be stored
local outputDirectory = comp:MapPath('Temp:/Lightfielder/')
-- local outputDirectory = comp:MapPath('Temp:/Lightfielder/')
...
if platform == 'Linux' then
	print('[Lightfielder][Warning] This script only works with MacOS (AVFoundation) and Windows (DirectShow) capture libraries. Linux support will be added in time.')
	return
end
...
-- Example: setPreferenceData('Lightfielder.SendMedia.Format', 3, true)
...
-- Example: getPreferenceData('Lightfielder.SendMedia.Format', 3, true)
...
-- Live Video Snapshot Window Image as Base64 content
...
function HelpWindow()
	...
	-- Help Resources
	local URL = 'https://github.com/Lightfielder/Lightfielder-DaVinci-Resolve/'

	-- Build the new window
	win = disp:AddWindow({
		ID = 'HelpWin',
		TargetID = 'HelpWin',
		WindowTitle = 'Lightfielder Live Video Snapshot Help Window',
		...
		ui:VGroup{
			...
			ui:TextEdit{
				...
				HTML = [=[
<h1>About Lightfielder Video Snapshot</h1>
<h1>]=] .. tostring(_VERSION) .. [=[</h1>

<p>The "Lightfielder Video Snapshot" script allows you to capture imagery from an HDMI/SDI/USB/Thunderbolt video capture device using the MacOS (AV Foundation) and Windows (DirectShow) libraries. The script has a UI Manager GUI and uses FFMPEG from the command line to do the capture task.</p>

<p>The script is compatible with Fusion 9-21.1+ and Resolve 1521.1+.</p>
...
<p>Step 1. Select the "Utility &gt; Lightfielder &gt; Tools &gt; Live Video Snapshot" menu item in Fusion to launch the script.</p>

<p>Step 2. On the top row of the Live Video Snapshot window you can select the "Video Input Device", then you can select the captured "Media Type", "Resolution", "FPS" (Frame Rate), and image buffer "Format". Make sure to adjust the "Capture Duration" value to define if you want single frame or multi-frame recording.</p>
...
<h2>Edit</h2>
<p>The "Edit" button loads the "Live Video Snapshot.lua" script in the default script editor program that is defined in the Fusion Preferences "Global and Default Settings &gt; Script &gt; Editor Path" section.</p>
...
function VideoDeviceList()
	...
	PathMapTable[6] = {id = 6, path = 'Temp:/Lightfielder/'}
	...
	-- FFmpeg capture log
	videoDevicesOutputLog = app:MapPath('Temp:/Lightfielder/LightfielderVideoDevicesLog.txt')
	...
	elseif platform == 'Windows' then
		...
		-- Check if the "Temp:/Lightfielder/" log folder exists. Create this folder if required
		logDir = app:MapPath('Temp:/Lightfielder/')
		...
		if ffmpegMissing == false then
			...
			videoDevicesBatPath = app:MapPath('Temp:/Lightfielder/LightfielderVideoDevices.bat')
			videoDevicesBatFP = io.open(videoDevicesBatPath, "w")
			if videoDevicesBatFP ~= nil then
				videoDevicesBatFP:write('@echo off\n')
				videoDevicesBatFP:write('@title Lightfielder Live Video Snapshot\n')
				...
				videoDevicesBatFP:write('echo echo The results are automatically loaded into the Live Video Snapshot GUI.\n')
				videoDevicesBatFP:write('echo ----------------------------------------------------------------------\n\n')
			...
			-- Run the LightfielderVideoDevices.bat script
			...
			-- Open the "Temp:/Lightfielder/LightfielderVideoDeviceLog.txt" logfile saved by Wintee
...
function FrameCapture(outputFilenamePrefix)
	...
	outputDirectory = cmp:MapPath(itm.PathMapCombo.CurrentText) or cmp:MapPath('Temp:/Lightfielder/')
	...
	-- Example: Temp:/Lightfielder/Screenshot.0001.jpg
	...
	if platform == 'Windows' then
		-- Running on Windows
		-- FFmpeg capture log
		outputLog = cmp:MapPath('Temp:/Lightfielder/LightfielderCaptureLog.txt')
		...
		-- Write out a .bat script to do the capture and collect the CLI stdin/stdout/stderr text output 
		captureBatPath = cmp:MapPath('Temp:/Lightfielder/LightfielderCapture.bat')
		captureBatFP = io.open(captureBatPath, "w")
		if captureBatFP ~= nil then
			captureBatFP:write('@echo off\n')
			captureBatFP:write('@title Lightfielder Live Video Snapshot\n')
			captureBatFP:write('setlocal EnableDelayedExpansion\n\n')
			captureBatFP:write('echo Live Video Snapshot Capture Script' .. _VERSION .. '\n')
		...
		if ffmpegMissing == false then
			...
			-- Run the LightfielderCapture.bat script
			...
			-- Open the "Temp:/Lightfielder/LightfielderCaptureLog.txt" logfile saved by Wintee
	...
	itm.SnapshotWin.WindowTitle = 'Lightfielder Live Video Snapshot |' .. ' Take ' .. tostring(takeNumber)
...
FilenamePrefix = getPreferenceData('Lightfielder.VideoSnapshot.FilenamePrefix', 'Snapshot', printStatus)

-- Capture sequence duration - Example: "1"
durationFrames = getPreferenceData('Lightfielder.VideoSnapshot.DurationFrames', tonumber(1), printStatus)

-- Camera warmup delay in seconds - Example: "0.5" seconds or "2" seconds (for USB webcams)
warmupSeconds = getPreferenceData('Lightfielder.VideoSnapshot.WarmupSeconds', tonumber(0.5), printStatus)

-- Should files be overwritten each time a capture is done? - Example: "0" for false, and "1" for true
overwriteMedia = getPreferenceData('Lightfielder.VideoSnapshot.OverwriteMedia', tonumber(0), printStatus)

-- Initial PathMap index - Example: "1" ("Comp:/Media/")
defaultPathMap = getPreferenceData('Lightfielder.VideoSnapshot.PathMap', tonumber(1), printStatus)

-- Video Input Device index
defaultVideoDevice = getPreferenceData('Lightfielder.VideoSnapshot.VideoDevice', tonumber(1), printStatus)

-- Image or Movie format index
defaultMediaType = getPreferenceData('Lightfielder.VideoSnapshot.MediaType', tonumber(1), printStatus)

-- Resolution index
defaultResolution = getPreferenceData('Lightfielder.VideoSnapshot.Resolution', tonumber(6), printStatus)

-- FPS index = 30 FPS
defaultFrameRate = getPreferenceData('Lightfielder.VideoSnapshot.FPS', tonumber(4), printStatus)
...
-- ffmpegProgram = '"' .. getPreferenceData('Lightfielder.SendMedia.FFmpegFile', defaultFFmpegProgram, printStatus) .. '"'
...
local width,height = 1270,350
win = disp:AddWindow({
	ID = 'SnapshotWin',
	TargetID = 'SnapshotWin',
	WindowTitle = 'Lightfielder Live Video Snapshot',
...
function win.On.FilenamePrefixText.TextChanged(ev)
	setPreferenceData('Lightfielder.VideoSnapshot.FilenamePrefix', tostring(itm.FilenamePrefixText.Text), printStatus)
...
function win.On.DurationText.TextChanged(ev)
	setPreferenceData('Lightfielder.VideoSnapshot.DurationFrames', tonumber(itm.DurationText.Text), printStatus)
...
function win.On.WarmupText.TextChanged(ev)
	setPreferenceData('Lightfielder.VideoSnapshot.WarmupSeconds', tonumber(itm.WarmupText.Text), printStatus)
...
function win.On.EditButton.Clicked(ev)
	scriptPath = app:MapPath('Lightfielder:/Resolve/Scripts/Utility/Lightfielder/Multi-View Camera Rigs/Live Video Snapshot.lua')
...
function win.On.OverwriteButton.Clicked(ev)
	...
	setPreferenceData('Lightfielder.VideoSnapshot.OverwriteMedia', tonumber(overwriteMedia), printStatus)
...
function win.On.PathMapCombo.CurrentIndexChanged(ev)
	...
	-- Fallback to comp:MapPath('Temp:/Lightfielder/') if this field is empty
	outputDirectory = comp:MapPath(itm.PathMapCombo.CurrentText) or comp:MapPath('Temp:/Lightfielder/')
	setPreferenceData('Lightfielder.VideoSnapshot.PathMap', tonumber(itm.PathMapCombo.CurrentIndex), printStatus)
end
...
function win.On.VideoDevicesCombo.CurrentIndexChanged(ev)
	...
	setPreferenceData('Lightfielder.VideoSnapshot.VideoDevice', tonumber(itm.VideoDevicesCombo.CurrentIndex), printStatus)
end

function win.On.VideoResolutionCombo.CurrentIndexChanged(ev)
	print('[Video Input] ', tostring(itm.VideoDevicesCombo.CurrentText) .. ' @ ' .. tostring(itm.VideoResolutionCombo.CurrentText))

	setPreferenceData('Lightfielder.VideoSnapshot.Resolution', tonumber(itm.VideoResolutionCombo.CurrentIndex), printStatus)
end

function win.On.VideoFrameRateCombo.CurrentIndexChanged(ev)
	print('[FPS] ', tostring(itm.VideoFrameRateCombo.CurrentText))

	setPreferenceData('Lightfielder.VideoSnapshot.FPS', tonumber(itm.VideoFrameRateCombo.CurrentIndex), printStatus)
end
...
function win.On.MediaTypeCombo.CurrentIndexChanged(ev)
	...
	setPreferenceData('Lightfielder.VideoSnapshot.MediaType', tonumber(itm.MediaTypeCombo.CurrentIndex), printStatus)
end
