...
1. Open a Resolve Edit page based timeline. Select the "Workspace > Script > Lightfielder > 13 Camera Contact Sheet" menu item. A "Camera Contact Sheet" window will appear.
...
def SavePresetWindow():
	...
	preset_dlg = preset_disp.AddWindow({
		...
		"ID": "PresetWin",
		"TargetID" : "PresetWin",
		"Geometry": [0, 185, 430, 130],
		"MinimumSize": [0, 185, 430, 130],
		"FixedSize": [0, 185, 430, 130],
		"Spacing": 0,
	},[
...
def CreateArrayWindow():
	...
	def SetJSON(presetName, data):
		...
		try:
			...
			print("\t[Lightfielder][JSON Save Error]", error)
...
# 				]),
	...
	def ShowPresetButtonFunc(ev):
		ShowFolderFromFilepath(app.MapPath("Scripts:/Utility/Lightfielder/Presets/Array/"))
	...
	def LoadCurrentLayoutOnFirstRun():
		...
		if project is None:
			...
			if timeline is None:
				...
				if timelineVideoTrackCount is not None:
					presetName = itm["PresetCombo"].CurrentText
					if presetName == "Active Layout":
						for i in range(1,int(timelineVideoTrackCount) + 1):
							camName = GetCameraNumber(i)
							trk = timeline.GetIsTrackEnabled("video", i)
							if (trk is not None) and (camName is not None):
								itm[camName].Checked = trk
