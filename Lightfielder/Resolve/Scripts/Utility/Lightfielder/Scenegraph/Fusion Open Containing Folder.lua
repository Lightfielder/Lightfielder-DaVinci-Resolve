...
Lightfielder Fusion Open Containing Folder - 2026-09-19 01.02 AM (UTC -3)
by Andrew Hazelden <andrew@andrewhazelden.com>

# Overview:

The "Fusion Open Containing Folder" script reads the active Nodes view selection and opens a desktop Explorer/Finder/Nautilus file browser window to show the containing folder that holds the selected media.

This script works with the following types of nodes in the Fusion 9-21.1+ and Resolve 15-21.1+ Fusion page Nodes view:
...
function Main()
	...
	if not fusion then
		print('[Lightfielder][Error] This is a Blackmagic Fusion Lua script. It should be run from within Fusion.')
		return
	end
	...
	if selectedNode then
		...
		elseif toolAttrs.TOOLS_RegID == 'Fuse.LifeSaver' then
			...
			if selectedNode and selectedNode.Output[comp.CurrentTime] and selectedNode.Output[comp.CurrentTime].Metadata and selectedNode.Output[comp.CurrentTime].Metadata.Filename then
				...
				result = '[Lightfielder][Invalid Node Type] '
			end
		end
		...
		if mediaDirName then
			...
			if not bmd.direxists(mediaDirName) then
				...
				print('[Lightfielder][Created Folder] ' .. mediaDirName .. '\n')
			end
			...
			if bmd.fileexists(mediaDirName) then
				...
				print('[Lightfielder][Folder Missing] ' .. mediaDirName .. '\n')
				return
			end
		end
	else
		print('[Lightfielder][Open Containing Folder] No media node was selected. Please select a node in the Flow view and run this script again.')
		return
	end
end
