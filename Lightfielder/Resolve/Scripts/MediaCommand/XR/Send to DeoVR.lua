--[[--
Send to DeoVR 2026-09-18 09.24 PM
...
function UserSettings()
	...
	win = disp:AddWindow({
		ID = "SendToDeoVRWin",
		TargetID = "SendToDeoVRWin",
		WindowTitle = "Send to DeoVR",
		...
		ui:VGroup{
			...
			ui:Button{
				...
				IconSize = {200,94},
				Icon = ui:Icon{
					File = "Lightfielder:/Resolve/Scripts/Support/KartaVR_Logo.png"
				},
				MinimumSize = {
					200,
					94,
			...
			ui:TextEdit{
				Weight = 1.0,
				ID = "InfoTxt",
				Text = "This script opens up a new DeoVR session. The currently selected images from the Media Command window will be auto-loaded into DeoVR.",
	...
	function win.On.SendToDeoVRWin.Close(ev)
    ...
    itm.SequenceCombo.CurrentIndex = 1
	...
	app:AddConfig("SendToDeoVRWin", {
		Target {
			ID = "SendToDeoVRWin",
		},

		Hotkeys {
			Target = "SendToDeoVRWin",
			Defaults = true,
	...
	app:RemoveConfig("SendToDeoVRWin")
	collectgarbage()
...
function OpenProgram(media, prefs)
	...
	if platform == "Windows" then
		-- Running on Windows
		viewerProgram = "C:\\Program Files (x86)\\Steam\\steamapps\\common\\DeoVR Video Player\\DeoVR.exe"
	...
	elseif platform == "Mac" then
		-- Running on Mac
		viewerProgram = "/Applications/DeoVR.app"
		command = 'open -a "' .. viewerProgram .. '" --args ' .. media

        print("  [Warning] DeoVR Player not been tested on macOS yet.")
		--print("\n[Launch Command] ", command)
		--os.execute(command)
	elseif platform == 'Linux' then
		-- Running on Linux
		print("  [Warning] DeoVR Player not been tested on Linux yet.")

		viewerProgram = "/opt/DeoVR/DeoVR"
